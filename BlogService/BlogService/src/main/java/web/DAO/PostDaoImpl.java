...
public class PostDaoImpl implements PostDao {
	...
	public Post getPost(String permalink) {
		...
		postObj.setLike(Integer.parseInt((String) resultPost.get("like")));
		...
		while (commIt.hasNext()) {
			...
			//commentObj.setLike(comm.getInt("like"));
	...
	public ArrayList<Post> showPost() {
		...
		System.out.println("document count is: "+collection.find().count());
		...
		while (cursor.hasNext()) {
			...
			postObj.setLike(tobj.get("like"));
			
			
			BasicDBList tag = (BasicDBList) tobj.get("tags");
			System.out.println("Null list is: "+(String)tobj.get("author"));
			System.out.println("Tag list is: "+tag);
			System.out.println("Size of the list is: "+tag.size());
			int size = tag.size();
			List<Comment> comment = new ArrayList<Comment>();
			...
			postObj.setTags(tags);
			commentList.setComments(comment);
			//postObj.setComments(comment);
			postObj.setCommentList(commentList);
	...
	public ArrayList<Post> getPostByTag(String tags) {
	   ...
	   BasicDBObject tagSearch= new  BasicDBObject("tags", tags);
      
		DBCursor cursor = collection.find(tagSearch);
		...
		while (cursor.hasNext()) {
			...
			System.out.println("Inside get post by tag service");
			postList.add(postObj);
			count++;
